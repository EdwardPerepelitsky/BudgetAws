...
router.post('/password', async (req, res, next) => {
    ...
    await gq.updateAccountInfo(userId, hashedNewPass, null, null, null);
...
router.post('/addenvelope', async (req, res, next) => {
    ...
    try{
        result = await Promise.all([gq.createNewEnvelope(userId, category, budget), 
            gq.updateAccountInfo(userId, null, null, availableBalance, false)])
    }
...
router.post('/removeenvelope', async (req, res, next) => {
    const userId = req.session.userId;
    if (!userId){
